...
- **可移植**：数据源三态检测 + PowerShell 5.1 告警 + 全 `$HOME` 相对（零硬写用户名/盘符）+ WorkBuddy Python 运行时版本无关（扫描 `versions/*` 取最高版）；工具仍**仅面向 Windows 设计**（详见「环境要求 → 操作系统」与「已知限制」），复制到别的 **Windows** 机器也能运行。
